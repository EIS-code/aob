...
class questionnaireController extends Controller
{
    ...
    public function index(Request $request, $id = null) {
        ...
        if(!empty($id)) {
            ...
            return view('admin.questionnaires-folders')->with(['data' => $data, 'folder' => $folder]);
        } else {
        ...
        return view('admin.questionnaires')->with(['data' => $data, 'folders' => $folders]);
    }
          ...
          if($_FILES['fileToUpload']['name']!=''){
            ...
            if($file){
                ...
                $ext = strtolower($ext);
                    ...
                    $id=0;
                    $updated_data = array('name' =>$file_name,'ext' =>$ext,'size' =>$size,'questionnaire_folder_id' =>session()->get('folder_id'),'uploaded_by' => sp_decryption(session()->get('admin_id')), 'created_at'=>date('Y-m-d h:i:s'));
