...
<div class="container">
    ...
    <div class="all-message">
    	<div class="al-title d-flex align-items-center justify-content-between">
        	<h5>All Messages<span class="count">{{$data['unread_messages']}}</span></h5>
            <!-- <div class="dots">...</div> -->
        </div>
        <ul class="nav nav-tabs user-list">
            <?php $i = 1;?>
            @foreach($data['users'] as $user)
            <?php $recent_chat = App\admin\Chat::getLastChatByUser(sp_decryption(session()->get('user_id')), $user->id);?>
        	<li>
                <a href="#userchat{{$user->id}}" <?php if (isset($_GET['userid'])) {if ($_GET['userid'] == $user->id) {echo 'class="user-lnk"';}} else {?> class="<?=($i == 1) ? 'active' : ''?><?php }?> user-lnk" data-toggle="tab" data-id="{{$user->id}}">
                    <div class="msg-body d-flex">
                        ...
                        <div class="msg-right">
                        @if($user->userUnreadMessages != 0)
                            <span class="user_unread_msg_{{$user->id}}">{{$user->userUnreadMessages}}</span>
                        @endif
                            <small>
                                @if($recent_chat)
                                    <?php
...
?>
    ...
    <div class="chat-right">
                            <div class="tab-content">
                                <?php $i=1; ?>
                                @foreach($data['users'] as $user)
                                <?php $chats = App\admin\Chat::getChatByUser(sp_decryption(session()->get('user_id')),$user->id); ?>
                                    <div class="tab-pane <?php if(isset($_GET['userid'])){ if($_GET['userid']==$user->id){ echo 'active'; } }else{?> <?=($i==1)?'active':''?><?php } ?>" id="userchat{{$user->id}}">
                                        ...
                                        <div class="cta-content">
                                            ...
                                            <span class="btn btn-success fileinput-button"><span class="img-up"><i class="fas fa-paperclip"></i></span>
...
function sendMessage(userid){
    ...
    if(msg==''){
        ...
        $.ajax({
            url: '<?php echo SYSTEM_SITE_URL; ?>user/sendMessage',
            ...
            success: function (result){
                ...
                if(parseInt(result.code)==1){
                    ...
                    $("#cta-cont-"+userid).append('<li><div class="ct-img"><figure><img src="{{SYSTEM_SITE_URL}}public/users/'+image+'" alt=""/></figure></div><div class="ct-right"><div class="ct-top">{{$data["user"]->name}}<span>{{date("H:i A")}}</span></div><div class="ct-bottom"><p>'+msg+'</p></div></div></li>');
                    getUsersListForChat();
                }else{
...
function getUsersListForChat(){
    ...
    $.ajax({
        url: '<?php echo SYSTEM_SITE_URL; ?>ajax/getUpdatedUsersListForUsers',
        ...
        success: function (result){
            if(parseInt(result.code)==1){
                ...
                $.each(data, function( index, value ) {
                    ...
                    if(value.userUnreadMessages != 0) {
                        html += '<span class="user_unread_msg_'+value.id+'">'+value.userUnreadMessages+'</span>';
                    }
                    html += '<p>';
...
$(document).ready(function() {
    ...
    $(document).on('click', '.user-lnk', function() {
        ...
        console.log(id);
        $.ajax({
            url: '<?php echo SYSTEM_SITE_URL; ?>ajax/readUserMessages',
